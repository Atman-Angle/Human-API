...
8. 不要重复用户的原始问题，直接开始报告内容。`,
      ...
      return {
        report,
        limitations: [
          "LLM-generated report; may reflect model biases and incomplete coverage of all sources.",
        ],
        run: {
          runId: randomUUID(),
  ...
  async synthesizeReport(
    ...
    try {
      if (!this.live.synthesizeReport)
        throw new LLMAdapterError(
          "LLM_UPSTREAM_UNAVAILABLE",
          "Live synthesizeReport is unavailable.",
        );
      ...
      const cached = this.cache.get(key);
      if (cached) {
        const report = (cached as { organization: unknown; run: typeof cached.run })
          .organization as never as string;
        return {
          report,
